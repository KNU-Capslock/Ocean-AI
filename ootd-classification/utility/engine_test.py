...
class Engine(object):
    ...
    def on_end_epoch(self, p, r):
        ...
        img_list = json.load(open('../data/kfashion_style/test_anno_custom_final_0.json', 'r'))
        ...
        df.to_csv('../output/style_correct_3.csv')
        ...
        df['pred'] = list(l_p_w)
        df.columns = ['img', 'real', 'pred']
        df.to_csv('../output/style_wrong_3.csv')
        ...
        df['pred'] = list(l_p_s)
        df.columns = ['img', 'real', 'pred']
        df.to_csv('../output/style_correct_5.csv')
        ...
        df['pred'] = list(l_p_w)
        df.columns = ['img', 'real', 'pred']
        df.to_csv('../output/style_wrong_5.csv')
    ...
    def learning(self, model, criterion, val_dataset):
        ...
        if self.state['evaluate']:
            a, b = self.validate(val_loader, model, criterion)
    ...
    def validate(self, data_loader, model, criterion):
            ...
            a, b = self.on_end_epoch(self.state['output'], self.state['target'])
